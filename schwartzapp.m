...
function onRun()
    ...
    if n <= 5
        uialert(fig, 'Неправильное чилсло узлов n', 'Неправильное n');
        return;
    end
    ...
    sum_h = h_L + h_R;
    main_diag = -2 ./ (h_L .* h_R);
    lower_diag = 2 ./ (h_L .* sum_h);
    upper_diag = 2 ./ (h_R .* sum_h);
    A = diag(main_diag) + diag(lower_diag(2:end), -1) + diag(upper_diag(1:end-1), 1); 
    rhs = f_vec(2:end-1);
    u_exact_inner = A \ rhs;
