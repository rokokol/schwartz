...
%% init
n = 101; a = 0; b = 1 * pi; u_0 = 0; u_n = 0; h = (b - a) / n;
intersect_points_radius = 2; accuracy = 1e-3 * 9;
x = linspace(a, b, n); f = sin(x)';

A = diag(ones(n - 3, 1), -1) + diag(ones(n - 3, 1), 1) - 2 * diag(ones(n - 2, 1));
% u_exact = A\(f(2:end-1) .* h^2);
u_exact = seidel(A, f(2:end-1) .* h^2, zeros([n-2, 1]), 1e-6, 100);
u_exact = [u_0 u_exact' u_n];

%% calculations
subplot(1, 3, 2); hold on; grid on;
[U, E] = test_schwartz_2d_symmetrical(n, a, b, intersect_points_radius, f, u_0, u_n, accuracy, true, 1);
